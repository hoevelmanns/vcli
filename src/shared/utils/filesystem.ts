...
export const createOrRenameSymlink = async (target: string, src: string): Promise<void> => {
  ...
  getPathByInode = async (inode: number): Promise<string> => await asyncExec(`find ~/ -inum ${inode} -print -quit`),
  getCwdInode = async (): Promise<string> => await fs.lstat(process.cwd()).then((stats: Stats) => stats.ino);
